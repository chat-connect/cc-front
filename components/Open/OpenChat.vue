<template>
    <div>
        ...
        <div v-for="item in getListOpenChat().reverse()" :key="item.openChatKey">
            <v-row v-if="item.myPost" justify="end" class="timeline">
                <v-card flat :key="item.openChatKey" class="timeline_item">
                    <v-row>
                        <v-col :cols="12">
                            <div class="user_info">
                        ...
                        <v-col :cols="12" >
                            ...
                            <div v-if="item.openChatImagePath != null">
                                <img class="chat_image" :src="item.openChatImagePath">
                            </div>
                        </v-col>
                    </v-row>
                </v-card> 
...
<script lang="ts">
import { FetchOpenChat } from '@/domain/usecase/fetchOpenChat';
...
export default {
    data() {
        return {
            content: "",
            openChatImage: null as File | null,
    ...
    methods: {
        ...
        async handleImageUpload(event) {
            ...
            this.openChatImage = encodedImage.replace(/^data:image\/\w+;base64,/, '');
        },
        // WebSocketサーバーに接続
        ...
        // チャット一覧を表示
        getListOpenChat() {
            const chat = this.listOpenChatStore.listOpenChat.items.list
            const chatList = [];

            const config = useRuntimeConfig();

            for (let i = 0; i < chat.length; i++) {
                var openChatImagePath = "";
                if (chat[i].image_path != "") {
                    openChatImagePath = `${config.public.GcImageUrl}/image/get?image_path=static/images/create_open_chat/${chat[i].open_chat_key}.png`
                }
                ...
                chatList.push({
                    openChatKey:       chat[i].open_chat_key,
                    ...
                    openChatImagePath: openChatImagePath,
        ...
        // チャット一覧を取得
        async listOpenChatHandler() {
            try {
                const route = useRoute();
                const userKey: string = this.userStore.user.items.user_key;

                const fetchOpenChat = new FetchOpenChat(ApiClient);
                const openChatList: ListOpenChat = await fetchOpenChat.listOpenChat(userKey);

                await this.listOpenChatStore.update(openChatList);
            } catch (error) {}
        ...
        async sendHandler() {
            if (this.websocket) {
                ...
                const messageObject = {
                    ...
                    open_chat_image: this.openChatImage,
                };
            ...
            this.content = "";
            this.openChatImage = null as File | null;
        },
        // scroll
        scrollChat() {
            try {
