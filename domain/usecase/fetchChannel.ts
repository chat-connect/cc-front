...
export class FetchChannel {
    ...
    async channelList(userKey: string, roomKey: string): ChannelList {
        ...
        const response: ChannelList = await this.apiClient.get("/api/channel/" + userKey + "/channel_list/" + roomKey, access_token.value)
    ...
    async channelCreate(body: any, userKey: string, roomKey: string): ChannelCreate {
        const access_token = useCookie('access_token')
        const response: ChannelCreate = await this.apiClient.post("/api/channel/" + userKey + "/channel_create/" + roomKey, body, access_token.value)
